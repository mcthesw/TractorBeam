...
fn connect_fake_hook(session: &HookIpcSession) -> LocalSocketStream {
    let name = session
        .endpoint
        .clone()
...
fn connect_with_retry(name: interprocess::local_socket::Name<'static>) -> LocalSocketStream {
    let deadline = Instant::now() + TEST_TIMEOUT;
    loop {
...
fn write_hook_message(stream: &mut LocalSocketStream, message: &HookToClient) -> io::Result<()> {
    let encoded = tractor_beam_hook_ipc::encode(message).map_err(protocol_io)?;
    write_all_bounded(stream, &encoded)
}

fn wait_for_shutdown(stream: &mut LocalSocketStream) {
    let mut decoder = FrameDecoder::new();
    let deadline = Instant::now() + TEST_TIMEOUT;
    loop {
...
fn packet(peer: u64, sequence: u32, payload: &[u8]) -> GamePacket {
    GamePacket {
        peer,
        sequence,
        channel: 3,
