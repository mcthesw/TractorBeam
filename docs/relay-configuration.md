...
请从仓库维护的 [`relay.toml`](../deploy/relay.toml) 开始配置。未知字段或无效值会
使 Relay 在启动时退出；没有明确需求时请保留默认值。
...
- 修改任一端口后，还要同步修改 Docker 端口映射，并在主机和云防火墙中放行
  对应的 TCP 或 UDP 端口。
...
- `pow_difficulty_bits` 控制加入房间时的工作量证明。公网 Relay 保持 `18`；只有
  可信的本地开发 Relay 才应使用 `0`。
...
接收端必须支持 OTLP/gRPC。每个运行中的 Relay 应使用稳定且唯一的
`service_instance_id`。导出信号参阅
