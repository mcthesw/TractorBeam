...
fn contributor_card(ui: &mut egui::Ui, contributor: Contributor) {
    ...
    pub(in crate::app) fn about_page(&mut self, ui: &mut egui::Ui) {
        ...
        for row in CONTRIBUTORS.chunks(columns) {
            ui.columns(columns, |column_ui| {
                for (column, contributor) in column_ui.iter_mut().zip(row.iter().copied()) {
                    contributor_card(column, contributor);
                }
            });
            ui.add_space(4.0);
        }
...
mod tests {
    ...
    fn acknowledgements_preserve_supplied_order_names_and_links() {
        ...
        assert_eq!(CONTRIBUTORS[2].name, "童心音乐会");
